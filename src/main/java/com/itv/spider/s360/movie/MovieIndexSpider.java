...
public class MovieIndexSpider extends AbstractSpider {
	...
	/**
	 * 将获得的电影分类 大陆，香港，搞笑...并让线程继续解析其页面并获得其页面的电影列表
	 * 由于360使用相对url所以要将本页面的url传到下一级
	 * v.360.cn/dianying
	 */
	public void run() {
		this.focusMap();
		...
		this.easyTop();
		this.passionTop();
		Map<String, String> map = MovieRegex.getMovieType(getPageInfo(this.url),
				url.substring(0, url.lastIndexOf('/')));
		if (map != null) {
			for (Entry<String, String> type_url : map.entrySet()) {
				if (type_url.getKey().indexOf("cat=") != -1) {//cat=xx 电影类型 喜剧，搞笑等连接参数,电影类型在其他页面无法获取
